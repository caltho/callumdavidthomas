 ...
 *
 * Material fatigue (`entropy`): stirring the word wears it out. The springs go
 * soft, so recovery gets slower, and the most stressed dots yield and settle a
 * spot or two from home for good. Wear heals by half every `wearHalfLife` ms.
 *
 * QA hooks in the query string: `?still` skips the intro, `?stir=n` runs n
 * synthetic cursor passes, `?poke=0.4,0.5` parks a fake cursor.
 */

export type EntropyReport = { moved: number; total: number; wear: number };
...
export type WordmarkOptions = {
  ...
  weight: number;
  tracking: number; // em
  lineGap: number; // em, between lines
  pad: number; // px around the text so displaced dots have room
  gap: number; // px between samples
  dot: number; // dot radius
  color: string;
  hot: string; // colour of a displaced dot
  radius: number; // pointer influence radius
  force: number;
  spring: number;
  damping: number;
  ring: boolean;
  introDuration: number; // ms; 0 disables the intro
  entropy: boolean;
  yieldMin: number; // stress every dot can take before it might yield
  yieldMean: number; // mean extra stress on top of that (exponential spread)
...
type Dot = {
  hx: number; hy: number; // current home
  ox: number; oy: number; // original home
  x: number; y: number;
  vx: number; vy: number;
  s: number; // accumulated stress
  lim: number; // stress this dot can take
  d: number; // intro delay, 0..1
  free: boolean; // yielded and coasting
  moved: boolean; // settled away from its original home
};

const DEFAULTS: WordmarkOptions = {
  lines: ["CALLUM THOMAS"],
...
export function createWordmark(el: HTMLElement, options: Partial<WordmarkOptions> = {}) {
  const o: WordmarkOptions = { ...DEFAULTS, ...options };
  ...
  canvas.style.transformOrigin = "50% 55%";
  ...
  let W = 0,
    ...
    visible = false,
    destroyed = false,
    introPending = o.introDuration > 0 && !reduced && !q.has("still") && !q.has("poke") && !q.has("stir");
  ...
  const key = (x: number, y: number) => Math.round((y - gy0) / o.gap) * 100000 + Math.round((x - gx0) / o.gap);
  ...
  function build() {
    ...
    const setFont = (c: CanvasRenderingContext2D, px: number) => {
      c.font = `${o.weight} ${px}px ${o.family}`;
      if ("letterSpacing" in c) c.letterSpacing = `${o.tracking * px}px`;
    };
    setFont(m, 100);
    const widest = Math.max(...lines.map((l) => m.measureText(l).width));
    const size = Math.floor((100 * (width - o.pad * 2)) / widest);
    setFont(m, size);
    const cap = Math.ceil(m.measureText("H").actualBoundingBoxAscent);
    ...
    c.textAlign = "center";
    lines.forEach((line, i) => c.fillText(line, W / 2, o.pad + cap + i * (cap + gapPx)));
    ...
    gx0 = ((W % g) + g) / 2;
    gy0 = g / 2;
    cols = Math.floor((W - gx0) / g) + 1;
    rows = Math.floor((H - gy0) / g) + 1;
    wear = moved = pending = 0;
    // Only keep samples inside each line's cap band, so the overshoot of round
    // letters (C, O, S) doesn't leave stray dots below the baseline.
    const bands = lines.map((_, i) => {
      const base = o.pad + cap + i * (cap + gapPx);
      return [base - cap - 0.5, base + 0.5];
    });
    for (let y = gy0; y < H; y += g) {
      if (!bands.some(([a, b]) => y >= a && y <= b)) continue;
      for (let x = gx0; x < W; x += g) {
        if (data[(Math.floor(y) * W + Math.floor(x)) * 4 + 3] > 110) {
          pts.push({ hx: x, hy: y, ox: x, oy: y, x, y, vx: 0, vy: 0, s: 0, lim: stressLimit(), d: Math.random(), free: false, moved: false });
        }
      }
    }
    occ = new Set(pts.map((p) => key(p.hx, p.hy)));

    if (introPending) renderIntro(0);
    else draw();
    if (visible && introPending) start();
    ...
    if (q.has("poke")) {
      const [fx, fy] = (q.get("poke") || "0.5,0.5").split(",").map(Number);
  ...
  function report() {
    ...
    setTimeout(() => {
      reported = false;
      if (!destroyed) o.onEntropy?.({ moved, total: pts.length, wear });
    }, 120);
  }

  // A yielded dot has come to rest: give it the nearest free grid spot, for
  // good. It moves the way it was pushed, but never more than maxDrift spots
  // from home, so the letters deform rather than scatter.
  function settle(p: Dot) {
    p.free = false;
    pending--;
    const lim = o.maxDrift,
      clamp = (v: number) => Math.max(-lim, Math.min(lim, v));
  ...
  function dotPath(path: Path2D, x: number, y: number, r: number) {
    path.moveTo(x + r, y);
  ...
  // Intro: the whole word glides forward from a distance (small, soft, faint)
  // while each dot fades in with a slight random lag. Pure easing, no springs,
  // so nothing overshoots or bounces.
  const LAG = 0.35;
  const ease = (x: number) => 1 - Math.pow(1 - x, 4);
  let t0 = 0;

  function renderIntro(k: number) {
    ...
    for (const p of pts) {
      const a = ease(Math.max(0, Math.min(1, k * (1 + LAG) - p.d * LAG)));
      if (a <= 0) continue;
  ...
  function introFrame(now: number) {
    if (!t0) t0 = now;
    const k = (now - t0) / o.introDuration;
    if (k < 1) {
      renderIntro(k);
  ...
  function start() {
    if (!pts.length || destroyed) return; // not built yet; build() starts the intro itself
    if (introPending) {
  ...
  const onMove = (e: PointerEvent) => {
    if (introPending) return; // let it land first
    const px = ptr.x,
      py = ptr.y,
      was = ptr.inside;
    local(e);
  ...
  const onDown = (e: PointerEvent) => {
    if (e.pointerType === "mouse" || introPending) return;
    local(e);
    for (const p of pts) {
      const dx = p.x - ptr.x,
        dy = p.y - ptr.y,
      ...
      if (d < o.radius * 1.6) {
        ...
        p.vx += (dx / d) * f;
        p.vy += (dy / d) * f;
        if (o.entropy) p.s += f * (0.25 + wear) * 3;
      }
    }
    stir(W / 4);
    start();
  ...
  // system font and the resize guard would stop it ever being redrawn.
  ...
  (document.fonts ? document.fonts.load(`${o.weight} 100px ${o.family}`, "CALLUM THOMAS").catch(() => {}) : Promise.resolve()).then(() => {
    if (destroyed) return;
    fontsOk = true;
    builtFor = -1;
    build();
